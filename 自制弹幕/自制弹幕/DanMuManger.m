...
- (void)createBulletComment:(NSString *)comment trajectory:(Trajectory)trajectory {
    ...
    //创建一个弹幕view
    DanmuView *view = [[DanmuView alloc] initWithContent:comment];
    //设置运行轨迹
    view.trajectory = trajectory;
    __weak DanmuView *weakBulletView = view;
    __weak DanMuManger *myself = self;
    /**
     *  弹幕view的动画过程中的回调状态
     *  Start:创建弹幕在进入屏幕之前
     *  Enter:弹幕完全进入屏幕
    ...
    if (self.MakeDanMuView) {
        self.MakeDanMuView(view);
    }
}
/**
 *  初始化弹幕
...
- (void)initBulletCommentView {
    //初始化三条弹幕轨迹
    NSMutableArray *arr = [NSMutableArray arrayWithArray:@[@(0), @(1), @(2),@(3),@(4)]];
